...
/**
 * A single invoice rendered as a self-contained card. Used by both the
 * single-invoice public page (`/r/[token]`) and the combined public page
 * (`/r/bulk?tokens=...`) so both look identical to the PDF.
 */
export function PublicReceipt({ sale }: { sale: PublicSale }) {
    ...
    >
      ...
      <div className="relative bg-[#0f172a] px-5 py-7 sm:px-10 sm:py-8">
        ...
        <div className="text-center">
          ...
          {tenant.description && (
            <p className="mt-1 text-xs italic text-slate-300">{tenant.description}</p>
          )}
      ...
      <div className="px-5 pt-7 text-center sm:px-10 sm:pt-8">
        <p className="text-[11px] font-bold uppercase tracking-[0.25em] text-slate-500">Invoice</p>
      ...
      <div className="grid grid-cols-1 gap-6 border-y border-border/60 px-5 py-5 sm:grid-cols-2 sm:px-10 sm:py-6">
        <div>
          <p className="text-xs font-bold uppercase tracking-wider text-foreground">Customer:</p>
            ...
            <div className="space-y-2 px-4 py-3.5">
              ...
              {sale.discount > 0 && (
                <div className="flex justify-between text-muted-foreground">
                  <span>
                    Discount
                    {(sale.discountPercent ?? 0) > 0 ? ` (${(sale.discountPercent ?? 0).toFixed(2)}%)` : ''}
                  </span>
                ...
                <>
                  {sale.taxItems.map((t) => (
                    <div key={t.name} className="flex justify-between text-muted-foreground">
                      <span>
                        {t.name}{' '}
                        <span className="text-[10px] text-muted-foreground/70">({t.rate}%)</span>
                      </span>
